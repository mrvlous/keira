...
//! Stack frame pointer walking and callstack backtrace unwinding.
//!
//! Traverses standard frame pointer linked lists (`RBP`/`EBP`) to produce
//! callstack traces for panic diagnostics, kernel profiling, and deadlock detection.
...
pub struct StackFrame {
    /// Return address / instruction pointer at this call depth.
    pub rip: u64,
    /// Base frame pointer address.
    pub rbp: u64,
}

/// Walks active kernel stack frame pointers using current processor registers.
#[inline(never)]
...
/// Walks kernel stack frame pointers beginning from an explicit `RBP`/`RIP` context.
#[inline(never)]
...
/// Unwinds and captures callstack frames from a given `RBP`/`RIP` frame pointer.
...
/// Unwinds the current active kernel callstack.
pub fn unwind_stack() {
    let mut frames = [StackFrame { rip: 0, rbp: 0 }; 16];
