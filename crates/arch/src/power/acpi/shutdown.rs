...
//! System power state management, ACPI soft poweroff, reset, and watchdog facilities.
...
/// Power off the system via ACPI sleep state S5 and platform power management registers.
pub fn poweroff() -> ! {
    unsafe {
        cli();
        outw(0x604, 0x2000);
