...
//! Stateful packet filtering engine, rules inspection, and Netfilter syscall handler.

#![allow(static_mut_refs)]

use keira_io::vga;

use crate::filter::bpf::BpfInstruction;
use crate::filter::firewall::conntrack::{ConnTrackEntry, MAX_CONNTRACK_ENTRIES};
use crate::filter::firewall::rule::{
    FirewallRule, MAX_FIREWALL_RULES, NETFILTER_CMD_ADD_RULE, NETFILTER_CMD_DEL_RULE,
    NETFILTER_CMD_FLUSH, NETFILTER_CMD_STATUS, NETFILTER_CMD_TOGGLE,
};
...
pub static mut RULE_TABLE: [FirewallRule; MAX_FIREWALL_RULES] = [
    FirewallRule {
...
pub static mut CONNTRACK_TABLE: [ConnTrackEntry; MAX_CONNTRACK_ENTRIES] = [ConnTrackEntry {
    proto: [0; 8],
    src_ip: [0; 16],
    dst_ip: [0; 16],
    dport: 0,
...
pub fn bpf_filter_packet(pkt: &[u8], insns: &[BpfInstruction]) -> bool {
    ...
    let res = crate::filter::bpf::bpf_run_filter(insns, pkt);
    res != 0
}
