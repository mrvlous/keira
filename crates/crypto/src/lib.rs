...
mod tests {
    ...
    fn test_tpm2_sealed_storage_roundtrip() {
        ...
        let mask = 1u32 << 16; // Dedicated PCR 16 for concurrent test isolation
        ...
        let unsealed_len =
            tpm::unseal_secret(&blob, &mut out_buf).expect("Unsealing should succeed");
    ...
    fn test_tpm2_sealed_storage_tamper_detection() {
        ...
        let mask = 1u32 << 17; // Dedicated PCR 17 for concurrent test isolation

        let blob = tpm::seal_secret(secret, mask).expect("Sealing should succeed");

        // Simulate tampering: malicious attacker or altered code extends PCR 17
    ...
    fn test_tpm2_measured_boot_primitives() {
        tpm::init();
        let fake_kernel_text = [0x90u8; 128]; // NOP sled
