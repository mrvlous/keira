...
//! Message Signaled Interrupt (MSI) discovery and Local APIC delivery configuration.

use crate::bus::pci::{pci_read_config_u32, pci_write_config_u32};

/// Finds the offset of the MSI Capability structure (Capability ID 0x05) in PCI configuration space.
...
/// Configures Message Signaled Interrupt (MSI) vector for a PCIe/PCI device targeting BSP APIC 0.
pub fn enable_msi(bus: u8, dev: u8, func: u8, vector: u8) -> Result<(), &'static str> {
    enable_msi_target(bus, dev, func, vector, 0)
}

/// Configures Message Signaled Interrupt (MSI) with a specific destination APIC CPU ID.
