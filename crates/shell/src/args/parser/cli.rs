...
//! CLI argument tokenization, flag checking, and option lookup routines.

use crate::args::flags::{CliArgs, MAX_ARG_TOKENS};

impl<'a> CliArgs<'a> {
    /// Parse arguments from a whitespace-split iterator.
    pub fn parse(parts: &mut core::str::SplitWhitespace<'a>) -> Self {
        let mut tokens = [""; MAX_ARG_TOKENS];
        let mut token_count = 0;

        for part in parts {
            if token_count < MAX_ARG_TOKENS {
