...
//! Service manager and runtime scheduler for background daemons.

use super::types::{ServiceRecord, ServiceState, MAX_SERVICES};
use crate::service::config::parser::parse_conf_into_record;
...
#[inline]
fn get_uptime() -> u64 {
    keira_arch::timers::get_uptime_ms()
}
    ...
    for i in 0..SERVICE_COUNT {
        ...
        if interval_ms > 0 && now >= SERVICES[i].last_tick_ms + interval_ms {
            ...
            if name == "syncd" {
                ...
                let _ = keira_io::rtc::get_time();
                SERVICES[i].log_event("RTC clock drift synchronized", now);
            } else if name == "monitord" {
                let (total_frames, alloc_frames, _free_frames) = keira_mem::pmm::get_stats();
