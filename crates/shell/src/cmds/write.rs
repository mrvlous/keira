...
pub fn run(parts: &mut core::str::SplitWhitespace) {
    unsafe {
        ...
        let file_exists = if let Ok((dir_cluster, name)) = keira_fs::fat::resolve_path(filename) {
        ...
        if is_append {
            match keira_fs::fat::append_file_content(filename, &text_buf[..text_len]) {
                Ok(new_size) => {
            ...
            match keira_fs::fat::write_file_content(filename, &text_buf[..text_len]) {
                Ok(_) => {
                    vga::set_color(vga::Color::LightGreen, vga::Color::Black);
