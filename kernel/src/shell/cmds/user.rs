...
fn ensure_passwd_exists() {
    unsafe {
        let _ = crate::fs::fat::create_dir("/system");
        let _ = crate::fs::fat::create_dir("/system/etc");
        let _ = crate::fs::fat::create_dir("/users");
        let _ = crate::fs::fat::create_file(PASSWD_PATH);

        let mut buf = [0u8; 1024];
        let existing = crate::fs::fat::read_file_content(PASSWD_PATH, &mut buf).unwrap_or(0);
        if existing == 0 {
            let default_content = b"admin:keira\n";
            let _ = crate::fs::fat::write_file_content(PASSWD_PATH, default_content);
        }
    }
}
...
pub fn lookup_user(username: &str) -> (bool, [u8; 16], usize) {
    unsafe {
        let mut buf = [0u8; 1024];
        let len = crate::fs::fat::read_file_content(PASSWD_PATH, &mut buf).unwrap_or(0);
    ...
    unsafe {
        ...
        match sub {
            ...
            Some("create") => {
                ...
                match crate::fs::fat::append_file_content(PASSWD_PATH, &line_buf[..offset]) {
                    Ok(_) => {}
                    Err(e) => {
                        vga::set_color(vga::Color::LightRed, vga::Color::Black);
                ...
                if let Ok(home_str) = core::str::from_utf8(&home_path_buf[..home_path_len]) {
                    let _ = crate::fs::fat::create_dir("/users");
                    let _ = crate::fs::fat::create_dir(home_str);
                }
            ...
            Some("delete") => {
                ...
                let _ = crate::fs::fat::write_file_content(PASSWD_PATH, &out_buf[..out_len]);
