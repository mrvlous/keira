...
//! Keira Kernel: Shell Command 'bg'
//!
//! Implementation of the 'bg' shell command to resume stopped process job in background.
        ...
        for i in 0..signal::JOB_COUNT {
            if let Some(ref mut job) = signal::JOB_TABLE[i] {
                if job.job_id == job_id {
                    ...
                    vga::print_str("Resumed Job [");
                    vga::print_u64(job.job_id as u64);
                    vga::print_str("] (PID ");
                    vga::print_u64(job.pid as u64);
                    vga::print_str(") in background execution context.\n");
                    break;
                }
            }
        }
