    ...
    if let Some(pipe_pos) = trimmed.find('|') {
        ...
        if !cmd1_part.is_empty() && !cmd2_part.is_empty() {
            ...
            unsafe {
                ...
                crate::io::vga::PIPE_BUFFER[..copy_len]
                    .copy_from_slice(&crate::io::vga::REDIRECT_BUFFER[..copy_len]);
...
pub fn execute_command_inner(cmd: &str) {
    ...
    match command {
        ...
        "power" | "poweroff" => super::cmds::power::run(&mut parts),
        "perf" => super::cmds::perf::run(&mut parts),
        "timer" => super::cmds::timer::run(&mut parts),
        "syslog" | "dmesg" => super::cmds::syslog::run(&mut parts),
        "kvm" => super::cmds::kvm::run(&mut parts),
        "nvme" => super::cmds::nvme::run(&mut parts),
        "ext4" => super::cmds::ext4::run(&mut parts),
        "cgroups" => super::cmds::cgroups::run(&mut parts),
        "futex" => super::cmds::futex::run(&mut parts),
        "bpf" => super::cmds::bpf::run(&mut parts),
        "tpm" => super::cmds::tpm::run(&mut parts),
            ...
            let found_in_path = unsafe {
                ...
                if prefix_sys.len() + cmd_bytes.len() < 64 {
                    ...
                    let in_fat = if let Ok((dir_cluster, filename)) =
                        crate::fs::fat::resolve_path(path_str)
                    {
