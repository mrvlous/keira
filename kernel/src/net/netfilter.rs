...
pub struct FirewallRule {
    pub chain: [u8; 12], // INPUT, OUTPUT, FORWARD, PREROUTING
    pub proto: [u8; 8],  // TCP, UDP, ICMP, ANY
    pub src_ip: [u8; 16],
    pub dst_ip: [u8; 16],
    pub dport: u16,
    pub action: [u8; 12], // ACCEPT, DROP, REJECT, MASQUERADE
    pub match_count: u32,
    pub in_use: bool,
}
...
pub struct ConnTrackEntry {
    ...
    pub dst_ip: [u8; 16],
    pub dport: u16,
    pub state: [u8; 12], // NEW, ESTABLISHED, RELATED
    pub packets: u32,
    pub in_use: bool,
}
...
/// System call vector 76: Stateful Netfilter Firewall & Packet Filtering
pub unsafe fn sys_netfilter(cmd: u32, _arg1: u64, _arg2: u64) -> Result<u64, &'static str> {
    match cmd {
        NETFILTER_CMD_STATUS => {
            ...
            for (idx, rule) in RULE_TABLE.iter().enumerate() {
                if !rule.in_use {
                    continue;
                }
            ...
            for conn in CONNTRACK_TABLE.iter() {
                if !conn.in_use {
                    continue;
                }
