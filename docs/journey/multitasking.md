...
* **Dynamic TSS RSP0 & Per-CPU Syscall Trampoline Switching**: Dynamic per-task `TSS.rsp0` and `PerCpu.kernel_stack` synchronization during scheduler context switches, ensuring privilege-level transitions use dedicated per-process kernel stacks without cross-core race conditions.
