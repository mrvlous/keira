...
| `/system/dev/urandom` | Character | Read (Entropy) | Non-blocking cryptographic pseudorandom byte stream |
| `/system/dev/ptmx` | Character | Read / Write | Pseudoterminal multiplexer clone device |
...
## Dynamic DevFS Isolation

Keira resolves device nodes dynamically through `dev::char::exists()` and `dev::char::read_dev_node()`. The VFS intercepts any access to `/system/dev/*` or `/dev/*` and routes it directly to kernel character device handlers. This allows virtual character devices (`null`, `zero`, `random`, `urandom`, `tty`, `ptmx`) to be opened with `O_RDONLY` and `O_RDWR` even if the underlying disk image does not contain backing directory entries or files for them.
