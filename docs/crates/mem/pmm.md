...
- **Per-CPU Recursive Locking Debug Detection**:
  - In debug mode, `PmmGuard::lock()` queries the actual Local APIC ID (`keira_arch::interrupts::get_current_lapic_id()`) and panics immediately if the same CPU attempts recursive acquisition, while permitting distinct cores to wait and acquire safely.
...
- **Invariant Checker**:
  - `verify_pmm_invariants()` and `verify_pmm_invariants_locked()` verify that the count of set bits in `ALLOCATION_BITMAP` exactly matches `USED_FRAMES_COUNT`, used memory never exceeds total memory, and every active node in `FREE_LIST_HEAD` is located inside a valid usable RAM region and marked free in the bitmap.
...
- `verify_pmm_invariants() -> Result<(), &'static str>`: Performs an exhaustive consistency check across the allocation bitmap, used frames counter, and memory boundaries (acquires PmmGuard).
- `verify_pmm_invariants_locked() -> Result<(), &'static str>`: Performs consistency check assuming the caller already holds the PMM spinlock.
